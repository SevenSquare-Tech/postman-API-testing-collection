...
const path = require("path"),
    ...
    heading = `// Type definitions for postman-collection ${pkg.version}
...
module.exports = function (exit) {
    ...
    exec(
        `${IS_WINDOWS ? "" : "node"} ${path.join(
            "node_modules",
            ".bin",
            "jsdoc"
        )}${IS_WINDOWS ? ".cmd" : ""}` + " -c .jsdoc-config-type-def.json -p",
        function (code) {
            if (!code) {
                fs.readFile(
                    `${TARGET_DIR}/index.d.ts`,
                    function (err, contents) {
                        if (err) {
                            console.info(
                                chalk.red.bold(
                                    "unable to read the type-definition file"
                                )
                            );
                            exit(1);
                        }
                        ...
                        fs.writeFile(
                            ...
                            function (err) {
                                if (err) {
                                    console.info(
                                        chalk.red.bold(
                                            "unable to write the type-definition file"
                                        )
                                    );
                                    exit(1);
                                }
                                console.info(
                                    chalk.green.bold(
                                        `Type-definition file saved successfully at "${TARGET_DIR}"`
                                    )
                                );
                                exit(0);
                            }
